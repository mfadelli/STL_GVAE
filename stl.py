...
def eventually(x: Tensor, time_span: int) -> Tensor:
    ...
    """
    #print(x.size(),time_span)
    
    if time_span > x.shape[2]:
        #print('Time span too long')
        time_span= x.shape[2]
        if time_span==0:
            return 0
...
class Atom(Node):
    ...
    def __init__(self, var_index: int, threshold: realnum, lte: bool = False) -> None:
        ...
        self.lte: bool = lte #less than equal
...
class Globally(Node):
    ...
    def _quantitative(self, x: Tensor, normalize: bool = False) -> Tensor:
        z1: Tensor = self.child._quantitative(x[:, :, int(self.left_time_bound):], normalize)
        # z1 = z1[:, :, self.left_time_bound:]
        if self.unbound or self.right_unbound:
            if self.adapt_unbound:
                z: Tensor
            ...
            z: Tensor = -eventually(-z1, int(self.right_time_bound) - int(self.left_time_bound))
        return z
...
class Eventually(Node):
    ...
    def _quantitative(self, x: Tensor, normalize: bool = False) -> Tensor:
        ...
        if self.unbound or self.right_unbound:
            ...
            if self.right_time_bound - self.left_time_bound <= 0:
                raise ValueError(f"Invalid time span. Check time bounds.") 
...
class Until(Node):
    ...
    def _quantitative(self, x: Tensor, normalize: bool = False) -> Tensor:
        if self.unbound:
            ...
            timed_until: Node = And(Eventually(self.right_child, left_time_bound=self.left_time_bound,
                                                   right_time_bound=self.right_time_bound-1),
            ...
            #                            Eventually(Until(self.left_child, self.right_child, unbound=True),
            #                                       left_time_bound=self.left_time_bound, right_unbound=True))
            #And(Globally(self.left_child, left_time_bound=0, right_time_bound=self.left_time_bound),
            z: Tensor = timed_until._quantitative(x, normalize=normalize)
        return z
